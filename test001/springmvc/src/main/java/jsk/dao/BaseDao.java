...
public class BaseDao {
    ...
    // 加载数据库驱动  com.mysql.jdbc.Driver
        ...
        public static void main(String[] args) throws SQLException {
            List<List<Object>> x = getData("mysql",
                    "select * from db");
    ...
    /*protected Boolean executeUpdate(String sql) throws Exception {
        Boolean suc = true;
		try {
			if (conn == null) {
				conn = getConnection();
            ...
            for (int i = 0; i < args.length; i++) {
                if ("String".equals(types[i])) {
                    ...
                    preparedStatement.setInt(i + 1,
                            Integer.parseInt(String.valueOf(args[i])));
                } else {
    ...
    /**
     * 注意这个方法的参数，和上面的方法不同 ，方法中说关闭的对象也不相同
     * 关闭连接、事务的方法，供子类调用
     */
    ...
    /**执行后不关闭与数据库的连接 jiangfw_20170306
     * 调用jdbc执行sql update类语句，返回执行结果true/false
     *
     * @param sql
     * @return
	...
	public PreparedStatement getPreparedStatement(String sqltxt)
		...
		try {
			...
			throw new Exception("Get prepared statement error("
					+ e.getMessage()
